...
if (searchTerm) {
      filtered = filtered.filter(contact => {
        ...
        const email = (contact.email_c || '').toLowerCase();
        const company = (contact.company_c || '').toLowerCase();
...
<div className="w-12 h-12 bg-gradient-to-r from-primary to-blue-700 rounded-full flex items-center justify-center">
                  ...
                  <Badge variant={getStatusBadge(contact.status_c)} size="sm" className="absolute top-4 right-4">
                    {(contact.status_c || 'lead').replace(/\b\w/g, l => l.toUpperCase())}
                  </Badge>
                </div>
...
<h3 className="text-lg font-semibold text-slate-800 mb-1">
                ...
                <p className="text-slate-600 mb-2">{contact.company_c || 'No company'}</p>
                <p className="text-sm text-slate-500 mb-3">{contact.email_c || 'No email'}</p>
                <div className="flex items-center justify-between text-xs text-slate-400">
                  <span>Created {contact.CreatedOn ? format(new Date(contact.CreatedOn), "MMM dd, yyyy") : 'Unknown'}</span>
                  <span>Last activity {contact.last_activity_c ? format(new Date(contact.last_activity_c), "MMM dd") : 'None'}</span>
                </div>
              </Card>
            </motion.div>
          ))}
...
{contactActivities.slice(0, 3).map(activity => (
                    <div key={activity.Id} className="flex items-start space-x-3 p-3 bg-slate-50 rounded-lg">
                      <Badge variant={getActivityIcon(activity.type_c)} size="sm">
